    ...
    def fit(self, X, y, batch_size=100, learning_rate=1e-7, reg=1e-5,
        ...
        for epoch in range(epochs):
            ...
            for batch in batches_indices:
                loss_batch = linear_softmax(X[batch], self.W, y[batch])[0] + l2_regularization(self.W, reg)[0]
                gradient = linear_softmax(X[batch], self.W, y[batch])[1] + l2_regularization(self.W, reg)[1]
                self.W += -learning_rate*gradient
                batch_losses.append(loss_batch)
            
            loss = np.sum(batch_losses)
